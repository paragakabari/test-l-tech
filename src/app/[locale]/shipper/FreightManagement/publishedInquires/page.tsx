...
import Styles from "./publishedInquires.module.css"
import { useRouter } from 'next/navigation'
import Image from 'next/image'
import Total from '@/Component/publishedInquiresTabs/Total'
import Awarded from '@/Component/publishedInquiresTabs/Awarded'
import Pending from '@/Component/publishedInquiresTabs/Pending'
import RevisionRequested from '@/Component/publishedInquiresTabs/RevisionRequested'
import Drafts from '@/Component/publishedInquiresTabs/Drafts'
import Discarded from '@/Component/publishedInquiresTabs/Discarded'
    ...
    return (
        <div className='position-relative'>
            <div className={Styles.secondDashBoard}>
                <HeroSecond BoldText='Published Inquires' breadcrumb={breadcrumb} />
                            ...
                            <Row className="gy-4">
                                <Col lg={12}>
                                    <div className='d-flex justify-content-end gap-2 w-100'>
                                        <ButtonComp text='Post Inquiry' className='secondButton' />
                                        <ButtonComp text='New Custom REP' />
                                    </div>
                                </Col>
                                ...
                                <Col lg={12} md={12} sm={12} xl={12}>
                                    <div className={Styles.tableMainBg}>
                                        {changeNav === "Total" && <Total />}
                                        {changeNav === "Awarded" && <Awarded deleteModal={() => setModalShow(true)} />}
                                        {changeNav === "Pending" && <Pending />}
                                        {changeNav === "Revision Requested" && <RevisionRequested />}
                                        {changeNav === "Drafts" && <Drafts />}
                                        {changeNav === "Discarded" && <Discarded />}
                                    </div>
                                </Col>
                            </Row>
                        </Col>
